...
export function makeGraphrevfigauge(data, plant = []) {
    let defaultprops = {
        "allPlants":1,
        "canvas": "id",
        "dataFlag": 0,
            ...
            if (graphIDs) {
                graphIDs && graphIDs.length > 0 && graphIDs.map((item, index) => {
                    if (data.graphId == "" || (data.graphId != "" && data.graphId == item)) {
                        service.makeGraphrevfigauge({ ...data, graphId: item }).then(response => {
                            dispatch(compareMakeGraphfgauge({ ...data, graphId: item }, [], "makeGraphrevfigauge"));
                        dispatch({
