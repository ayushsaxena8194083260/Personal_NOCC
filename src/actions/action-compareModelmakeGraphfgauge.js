...
import { getPlantByProjectId } from "./PlantFaultDataActions";
...
function graphFormat(dataList) {
    ...
    dataList.forEach((data) => {
        ...
        result["title"] = "Date from: " + data.length > 0 && getDate(data.fromDate) + " to: " + getDate(data.toDate);
        result["subtitleShow"] = true;
        result["legend"] = true;
...
export function compareMakeGraphfgauge(data, plant = [], graphAPI) {
    let defaultprops = {
            ...
            if (graphIDs) {
                graphIDs && graphIDs.length > 0 && graphIDs.map((item, index) => {
                    if (data.graphId == "" || (data.graphId != "" && data.graphId == item)) {
                        if (graphAPI == "makeGraphfgauge") {
                            service.comparemakeGraphfgauge({ ...data, graphId: item }).then(response => {

                                commonResopnse(dispatch, response, index, data.graphId, data.subPageID,data);

                            })
                        }
                        else  if (graphAPI == "makeGraphrevfigauge") {
                            service.comparemakeGraphrevfigauge({ ...data, graphId: item }).then(response => {

                                commonResopnse(dispatch, response, index, data.graphId, data.subPageID,data);

                            })
                        }
                        else  if (graphAPI == "makeGraphplf") {
                            service.comparemakegraphplf({ ...data, graphId: item }).then(response => {

                                commonResopnse(dispatch, response, index, data.graphId, data.subPageID,data);

                            })
                        }
                        else  if (graphAPI == "makeGraphrevfgauge") {
                            service.comparemakeGraphrevfgauge({ ...data, graphId: item }).then(response => {

                                commonResopnse(dispatch, response, index, data.graphId, data.subPageID,data);

                            })
                        }
                        else  if (graphAPI == "makeGraphfyrgauge") {
                            service.comparemakeGraphfyrgauge({ ...data, graphId: item }).then(response => {
...
function commonResopnse(dispatch, response, index, graphId, subIndex,data)
{
    dispatch({
        type: GET_COMPMODL_GAUGE_MONTHLY_REPORT_RESPONSE,
        payload: { "graphNo": "compareGaugeGraphs" + index, "canvas": data.graphId, response: graphFormat(response.data) }
    })
    let arr = [];
    response.data && response.data.monthlySeriesList && response.data.monthlySeriesList.length > 1 && Object.keys(response.data.monthlySeriesList[0]).forEach(function (key) {
