...
export function makeGraphplf(data, plant = []) {
    let defaultprops = {
        "canvas": "id",
        "dataFlag": 0,
        "externalDate": "string",
        ...
        if (data.pageID && data.subPageID) {
            ...
            if (graphIDs) {
                graphIDs && graphIDs.length > 0 && graphIDs.map((item, index) => {
                    ...
                    service.makeGraphplf({ ...data, graphId: item }).then(response => {
                        dispatch(compareMakeGraphfgauge({ ...data, graphId: item }, [], "makeGraphplf"));
                        dispatch({
