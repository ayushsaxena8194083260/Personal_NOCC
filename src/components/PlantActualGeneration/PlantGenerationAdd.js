...
import { Modal, Button, Row, div, Form, } from 'react-bootstrap';
...
import {PlantGeneration} from './PlantGeneration'
class PlantGenerationAdd extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      isSubmitted: false,
      selectedPlantOptions: this.props.postData.plantId
    }
  ...
  getPlantId(plantName, plants) {
    let plntNm = plants.filter((item) => item.displayText === plantName)[0];
    return plntNm.value;
  }
  ...
  renderErrortext(fieldID, msg) {
    return (
      this.state.isSubmitted === true && !this.state.postData[fieldID] && <p className="errorText" style={{ divor: "#CC0000", display: "block", fontSize: "11px", marginTopop: "0.5em" }}>{msg}</p>
    )
  }
  ...
  onSubmit = () => {
    ...
    const modal = {
      "actualGeneration": this.state.postData.export,
      "autoGenerated": 0,
      "bitFlag": 0,
      "connectedCapacity": "",
      "dataFlag": 1,
      "date": date,
      "gridOutage": "",
      "import_data": this.state.postData.importData,
      "jmrDate": date,
      "jmrStart": date,
      "ndays": 0,
      "export": 0,
        ...
        <Modal {...this.props} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
          ...
          <Modal.Body>
            ...
            <Col  lg={2} md={2} sm={6}>
                        ...
                        <Col lg={3} md={3} sm={6} className="xinput_group full__width">
                <select required class="top-search-input form-control" name="dataFlag" type="dropdown" onChange={(item) => this.handleChange(item)} value={this.state.postData && this.state.postData.dataFlag}>
            ...
            <Col  lg={2} md={2} sm={6}>
              ...
              <Col  lg={2} md={2} sm={6}>
                        ...
                        <Col lg={3} md={3} sm={6} className="xinput_group full__width">
                <Form.Control className="top-search-input" name="importData" type="text" onChange={(item) => this.handleChange(item)} value={this.state.postData && this.state.postData.importData} />
                {this.renderErrortext("importData", "The Import (KWH) is Required.")}
            </Col>
...
const mapStateToProps = (state, props) => {
  ...
  return {
    ...
    submitedFields: ["pyranometerTilt", "importData", "plantId", "dataFlag", "export", "date", "tilt", "connectedCapacity"]
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(PlantGenerationAdd));