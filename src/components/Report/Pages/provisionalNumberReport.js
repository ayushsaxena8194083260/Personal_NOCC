...
import {getProvisionalReportGeneration} from '../../../actions/action-Report';
import { connect } from 'react-redux';
import Picky from 'react-picky';
// import * as FileSaver from 'file-saver';
// import * as XLSX from 'xlsx';

class ProvisionalNumberReport extends Component{
    ...
    downloadData(){
        var plantIds = [];
        if(this.state.selectedPlantOptions.length>1){
        for(var i=0;i<this.state.selectedPlantOptions.length;i++){
            plantIds.push(this.state.selectedPlantOptions[i].id);
        }
        }
        else{
            plantIds.push(this.state.selectedPlantOptions.id);
        }
        this.props.getProvisionalReportGeneration({plantIds,fromDate:this.state.selectedFromDate,toDate:this.state.selectedToDate});
        //this.state.reportData && this.state.reportData.length > 0 && this.exportToCSV(this.state.reportData, "provisionalReport");
    }
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        getProvisionalReportGeneration: (data) => dispatch(getProvisionalReportGeneration(data))
    }
}

export default withRouter(connect(mapStateToProps, mapDispatchToProps)(ProvisionalNumberReport));