...
export default {
    exportCsv(response) {
        ...
        if ((parseInt(response.graphId) == 960) || (parseInt(response.graphId) == 201) || (parseInt(response.graphId) == 925) ||
        (parseInt(response.graphId) == 926) || (parseInt(response.graphId) == 927) || (parseInt(response.graphId) == 928) ||
        (parseInt(response.graphId) == 930) || (parseInt(response.graphId) == 935) || (parseInt(response.graphId) == 936) || 
        (parseInt(response.graphId) == 937) ){          
        let outputArray = []; 
        let resultArr= isJson(response.result)?JSON.parse(response.result):response.result
        let firstArray = resultArr[0];
