...
class FTPWeatherMappingEdit extends Component {
    constructor(props) {
        super(props);
        this.state = { 
            ...
            // postData: this.props.location.ftpWethMapp,
            postData: this.props.location && this.props.location.ftpWethMapp? this.props.location.ftpWethMapp : {},
    ...
    onSubmit = (e) => {
        ...
        if (this.state.postData != null) {
            ...
            this.props.history.push('/setting/FTPWeatherMapping');
        }
    }
                        ...
                        <Card.Body>
                            ...
                            <Row>
                                ...
                                <select class="form-control" name="adapterId" type="dropdown" onChange={(item) => this.handleChangeAdapterNme(item)}>
                                        {this.props.alternateAdapter && this.props.link != 'Add' && this.props.alternateAdapter.map((item, key) => {
                            ...
                            <Row>
                                ...
                                <Col>
                                    <Form.Control name="sourcePath" type="text" onChange={(item) => this.handleChange(item)} value={this.state.postData && this.props.link != 'Add' && this.state.postData.sourcePath} />
                                </Col>
                                <Col>
                                    <Form.Label>File Path<span className="form-required">*</span></Form.Label>
                                </Col>
                                <Col>
                                    <Form.Control name="filePath" type="text" onChange={(item) => this.handleChange(item)} value={this.state.postData && this.props.link != 'Add' && this.state.postData.filePath} />
                                </Col>
                            </Row>                            
                            <Row>
                                <Col>
                                ...
                                <Route render={({ history }) => (
                                    <Col md={2}>
                                        <Button variant="primary" size="md" onClick={() => { history.push('/setting/FTPWeatherMapping') }} block>Back</Button>
                                    </Col>
                                )} />
                            </Row>
                        </Card.Body>
...
const mapStateToProps = (state, props) => {
    return {
        pageName: props.location.ftpWethMapp ? props.location.ftpWethMapp.adapterId ? "Edit FTP" : "Add FTP" : null,
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(FTPWeatherMappingEdit));