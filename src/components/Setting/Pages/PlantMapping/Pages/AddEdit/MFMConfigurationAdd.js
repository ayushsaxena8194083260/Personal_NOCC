...
class MFMConfigurationAdd extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmit = (e) => {
        e.preventDefault();
        this.state.postData.mfmId = 0;
        this.props.createUpdateMFM({ mfmConf: this.state.postData, type: "Add MFM Configuration" });
            ...
            <form onSubmit={this.onSubmit}>
                <div>
                    <Card className="add-fault-card">
                        <Card.Header as="h5">Add MFM Configuration</Card.Header>
                        <Card.Body>
                            <Row>
                                <Col>
                                ...
                                <Col>
                                    <Form.Control name="meterNumber" type="text" onChange={(item) => this.handleChange(item)} value={this.state.postData && this.state.postData.meterNumber ? null : null} />
                                </Col>
                                <Col>
                                    <Form.Label>Multiply Factor:<span className="form-required">*</span></Form.Label>
                                </Col>
                                <Col>
                                    <Form.Control name="multiplyingFactor" type="text" onChange={(item) => this.handleChange(item)} value={this.state.postData && this.state.postData.multiplyingFactor ? null : null} />
                                </Col>
                            </Row>
                            <Row>
                            <Col></Col>
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(MFMConfigurationAdd));